...
router.get('/alta',function(req,res,next){
 ...
 router.post('/alta',upload.single('imagen'),async function(req,res,next){
  let sentencia = `insert into peliculas (titulo,sinopsis,portada) values ('
  ${req.body.titulo}','${req.body.sinopsis}','/images/${req.file.originalname}')`
...
if (req.file){
  ...
  sentencia =  `update peliculas set titulo  = '${req.body.titulo}', sinopsis  = '${req.body.sinopsis}', portada = '/images/${req.file.originalname}' 
  where id = ${req.params.id} `

  fs.createReadStream("./uploads/" + req.file.filename).pipe(fs.createWriteStream("./public/images/" + req.file.originalname), function(error){})

} else {
 sentencia = `update peliculas set titulo  = '${req.body.titulo}', sinopsis  = '${req.body.sinopsis}' where id = ${req.params.id}` 
}  
